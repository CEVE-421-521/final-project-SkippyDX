...
plot(2024:2065, last(test_tuple);
    ...
    title = "House Value Over Time\nCalculated vis 'For' Looop",
    label = false)
```

Due to concerns about the lack of visually apparent change in house appreciation at the included 25 year 0.5% decrease, we verified the housing appreciation by running the simulated house value appreciation with both a "for" loop, above, and an explicit expression of compounding value. The results were identical.

$$\[ P_t = \begin{cases}
250000 \times (1 + 0.045)^t, & \text{if } 0 \leq t < 25 \\
250000 \times (1 + 0.045)^{25} \times (1 + 0.04)^{t - 25}, & \text{if } 25 \leq t < 75 \\
250000 \times (1 + 0.045)^{25} \times (1 + 0.04)^{75 - 25} \times (1 + 0.035)^{t - 75}, & \text{if } t \geq 75
\end{cases} \]$$

```{julia}
#| echo: false
house_drs = map(2024:2065) do year
        if year - minimum(2024:2065) > 24
            house_dr = (1 + 0.045) ^ 25 * (1 + 0.045-.005) ^ (year - minimum(2024:2065) - 25) 

        elseif year - minimum(2024:2065) > 74
            house_dr = (1 + 0.045) ^ 25 * (1 + 0.045-.005) ^ 75 * (1+0.045 - 0.01) ^ (year - minimum(2024:2065) -75)
        else house_dr = (1 + 0.045) ^ (year - minimum(2024:2065))
        end
    end

house_values = [250000 .* house_drs]

plot(2024:2065, house_values;
    xlabel="Year",
    ylabel="House Value [USD]",
    title = "House Value Over Time\nCalculated Directly",
    label = false)
...
These simplified examples of the values resulting from the implementation of expected damage given scarcity and house value appreciation indicate that the additions to the simulation are operating as intended.
...
plot(
    ...
    marker=:circle,
    title = "NPV vs. Elevation Actions \nusing Different Model Features",
)
